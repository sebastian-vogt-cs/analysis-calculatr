...
pub fn get_zeros(func:&Vec<(bool, f64, usize)>, iterations:u16)->Vec<f64>{
    ...
    if func[0].2 == 1{
        values.push(round(newton_alg(0.0, func, iterations)));
        return values;
    ...
    //else get the extrema (zeros of derivative -> recursion until derivative is x¹).
        ...
        let extrema = get_zeros(&derive(func), iterations + 1);
        ...
        for x in extrema{
            if (get_y_for(x, func).is_sign_positive() && !(get_y_for(memory, func).is_sign_positive())) || (!(get_y_for(x, func).is_sign_positive()) && get_y_for(memory, func).is_sign_positive()) || (get_y_for(memory, func) == 0.0) || (get_y_for(x, func) == 0.0){
                let new_zero = round(newton_alg((x + memory) / 2.0, func, iterations));
        ...
        let new_zero = round(newton_alg(last_extreme + 1.0, func, iterations));
        if ((values.len() >= 1) && (new_zero != values[values.len()-1])) || (values.len() == 0){
                    values.push(new_zero);
         }
    }
...
fn newton_alg(x_start:f64, func:&Vec<(bool, f64, usize)>, iterations:u16)->f64{
    let mut x:f64 = x_start;
    let derivative:Vec<(bool, f64, usize)> = derive(func);
    for _i in 0..(((func[0].2 as f64 + iterations as f64).powf(2.0))/func[0].1.sqrt()) as usize{
